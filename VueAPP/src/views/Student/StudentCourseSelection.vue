  ...
  <div class="select-course">
    <div class="category-buttons">
      <el-button v-for="category in categories" :key="category.value"
        :type="selectedCategory === category.value ? 'primary' : 'default'" @click="selectCategory(category.value)"
        :style="{ width: '250px' }"> <!-- 设置按钮宽度-->
        {{ category.text }}
      </el-button>
    ...
    <el-collapse v-model="expandedPanels" accordion>
      <el-collapse-item v-for="(group) in groupWithNonEmptyTimes" :key="group.prefix" :name="`${group.prefix}_panel`"
        ...
        <div class="course-details">
          <div v-for="course in group.courses" :key="course.id" class="course-box"
            ...
            <template v-else>
              <el-button :type="course.studentNum < course.courseStorage ? 'primary' : 'danger'"
                :disabled="course.studentNum >= course.courseStorage" size="small"
                @click="selectCourse(course.id, course.courNumber)" :style="{ width: '80px' }">
                {{ course.studentNum < course.courseStorage ? '选课' : '已满' }} </el-button>
            </template>
          </div>
        </div>
      </el-collapse-item>
