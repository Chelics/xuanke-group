...
# 问题: 用户名命名规则, 密码长度规则
当前设定, 用户名9位, 第一位表示学生/教师/教务, 后面七位无意义; 密码6-20位, 不符合位数要求直接判定用户名或密码错误
...
另: 目前的排课很慢, 如果后期不能改进, 当教务一次性提交几十甚至上百个通过审核的课程时, 可能需要前端提示排课过程较长, 请耐心等待
...
**预检请求**是浏览器在执行跨源HTTP请求之前自动发起的一种特殊请求。其目的是为了保证安全性，确保跨源请求不会对服务器造成安全威胁。具体来说，当一个跨源请求包含某些特定条件时（如非简单请求），浏览器会首先发送一个OPTIONS类型的HTTP请求到目标服务器，询问服务器是否允许该跨源请求。

1. **什么是非简单请求？**
    - 非简单请求是指不符合以下条件的请求：
        - 请求的方法只能是`GET`、`POST`、`HEAD`之一。
        - 请求的`header`只能是`Accept`、`Accept-Language`、`Content-Language`、`Content-Type`这些字段，不能超出这些字段。
        - 对于请求的`header`的`Content-Type`字段，只能是以下值之一：`text/plain`、`multipart/form-data`、`application/x-www-form-urlencoded`。
    - 如果请求不满足以上条件，就被视为非简单请求。

2. **预检请求的流程：**
    - 如果是简单请求，浏览器会直接向目标服务器发出请求，如果请求符合CORS的配置，则返回结果。
    - 如果不是简单请求，浏览器会在发出真正请求前，先发出预检请求，检查当前请求是否符合服务器的CORS配置。如果符合，则再发出真正的请求；如果不符合，则直接返回跨域报错。

3. **为什么需要预检请求？**
    - 预检请求针对的是非简单请求，这些请求有可能会在服务器进行比较大的运算，增加负载。如果此时CORS不通过，就有可能增加了服务器没有必要的运算。预检请求的存在可以在一定程度上减少服务器无效的运算。

# 项目安全模块

## 密码安全
- 采用HTTPS的传输方式, 在传输过程中有效保障了密码的安全性
- 使用BCryptPasswordEncoder将数据加密后存储到数据库中, 不储存明文, 系统本身也不知道原来的密码是什么, 有效降低了数据库泄露导致的密码泄露风险
- 在密码比对过程中使用BCryptPasswordEncoder进行比对, 有效降低了密码比对过程中的泄露风险

## 身份认证安全
- 使用JWT令牌技术, 拦截未经登录的用户发送的请求, 减少了系统受到攻击的可能
- 采用HTTPS传输技术, 使用SSL证书来验证通信双方的身份，确保用户正在与预期的服务器进行通信，而非伪装的服务器。
- CORS跨域请求拒绝(没搞懂, 不讲)
